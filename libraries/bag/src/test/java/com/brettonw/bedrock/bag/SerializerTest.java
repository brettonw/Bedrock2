...
public class SerializerTest {
    ...
    @Test
    public void testUnopenedType () {
        // deal with a type that is not opened and must be serialized (Java 14)
    ...
    @Test
    public void testNoAccessibleDefaultConstructor () {
        var e = new TestClassE ("Hello World");
        var bagObject = Serializer.toBagObject (e);
        log.info (bagObject.toString ());
        TestClassE  reconE = Serializer.fromBagObject (bagObject);
        BagTest.report (e, reconE, "Reconstructed TestClassE should match the original");
    }
    ...
    @Test
    public void testSimpleDogfood () {
        var testClassC = new TestClassC (1, 2, 3.1f, 4, 5, 6.1f);
        var serializedTestClassC = Serializer.toBagObject(testClassC);
        var reconC = (TestClassC) Serializer.fromBagObject (serializedTestClassC);
        BagTest.report (reconC.getF (), testClassC.getF(), "Simple deserialization - f");
    }
    ...
    public void testIncompleteSimpleSerializer () {
        ...
        BagTest.report (c.getD (), 0, "Simple deserialization with missing items (as user intended)");
    }
}
